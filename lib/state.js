...
const SYNC_DEFAULTS = {
  enabled: true,
  notify: false,
  // When true (default), any domain that has at least one session cookie
  // is treated as "in use" during cleanup, so the entire domain's cookies
  // are preserved — not just the session one. This is a safety net for
  // sites that are actively using session cookies: even if a non-session
  // cookie in the same domain would technically be deletable as
  // third-party, we keep it to avoid breaking whatever the site is doing.
...
export async function setKeepSessionCookies(keep) {
  await chrome.storage.sync.set({ keepSessionCookies: !!keep });
}
